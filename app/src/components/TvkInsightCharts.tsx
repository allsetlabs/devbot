...
const ALLIANCE_DATA = [
  ...
  { name: 'DMK\nAlliance', seats: 60, fill: '#22c55e', label: 'DMK+INC+VCK+CPI+CPI(M)+IUML' },
  { name: 'ADMK\n(Solo)', seats: 56, fill: '#a3731a', label: 'ADMK' },
  { name: 'PMK &\nOthers', seats: 9, fill: '#94a3b8', label: 'PMK(5)+BJP(1)+DMDK(1)+AMMK(1)' },
];

// Pie data for alliance share of 234 seats
const ALLIANCE_PIE = [
  { name: 'TVK (Solo)', value: 109, color: '#e855a8' },
  { name: 'DMK Alliance', value: 60, color: '#22c55e' },
  { name: 'ADMK (Solo)', value: 56, color: '#a3731a' },
  { name: 'PMK & Others', value: 9, color: '#94a3b8' },
];
  ...
  return (
    <ChartCard title="Seat Share by Alliance (234 seats)">
      ...
      <p className="mt-2 text-center text-xs text-muted-foreground">
        TVK contested ALL 234 seats solo — unprecedented debut
      </p>
    </ChartCard>
  );
}
  ...
  return (
    <ChartCard title="Alliance vs TVK — Seat Comparison">
      ...
      <p className="mt-1 text-center text-xs text-muted-foreground">
        DMK Alliance = DMK(49)+INC(4)+VCK(2)+CPI(2)+CPI(M)(2)+IUML(1)
      </p>
    </ChartCard>
  );
}
...
export function TvkMajorityPathChart() {
  // Show TVK current + gap + potential in close losses
  const data = [
    { label: 'TVK Won', seats: 109, fill: '#e855a8' },
    { label: 'Majority', seats: 118, fill: '#94a3b8' },
    { label: '+Close <1K', seats: 119, fill: '#fbbf24' },
    { label: '+All Close', seats: 148, fill: '#f97316' },
  ];
  return (
    <ChartCard title="TVK Path to Majority — What-If Scenarios">
      ...
      <div className="mt-2 space-y-1 text-xs text-muted-foreground">
        <p>
          ...
          seats lost by &lt;1,000 votes → 119 (1 over majority)
        </p>
        <p>
          <span className="font-medium text-foreground">+All Close:</span> If TVK had won all 39
          seats lost by &lt;5,000 votes → 148 (30 over majority)
        </p>
      </div>
    </ChartCard>
  );
