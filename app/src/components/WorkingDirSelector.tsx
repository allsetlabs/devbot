    ...
    <div ref={dropdownRef} className="relative">
      ...
      <div className="relative">
        ...
        {dropdownOpen && savedDirs.length > 0 && (
          <div className="absolute bottom-full left-0 z-10 mb-1 max-h-48 w-full overflow-auto rounded-lg border border-border bg-background shadow-lg">
            {savedDirs.map((dir) => (
              <Button
                key={dir.id}
                variant="ghost"
                ...
                className="flex h-auto w-full items-start justify-start gap-2 whitespace-normal px-3 py-2.5 text-left hover:bg-muted"
              >
                <div className="min-w-0 flex-1">
                  <span className="block break-all text-sm text-foreground">{dir.path}</span>
                  {dir.label && <span className="text-xs text-muted-foreground">{dir.label}</span>}
                </div>
                {dir.isDefault && <Shield className="mt-0.5 h-3.5 w-3.5 shrink-0 text-primary" />}
                {dir.isRootDirectory && (
